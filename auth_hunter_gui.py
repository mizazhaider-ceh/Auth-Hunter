...
HELP_SECTIONS = [
    ...
    ('p', 'Two different ideas. The VALUES you try, and the form FIELD NAMES they '
          'go into.'),
    ('p', 'Values you try:  Single user (-l) or User list (-L), and Single '
          'password (-p) or Password list (-P). A Combo list is one user:pass per '
          'line. With no credentials, the built-in common list runs first.'),
    ('p', 'Field names: read them from the login form HTML. View Source (F12) and '
          'find the inputs:'),
    ...
    ('c', '</form>'),
    ('p', 'The names already default to username and password, so set "User field '
          'name" / "Pass field name" only when the form differs, e.g. name="email" '
          'means User field name = email.'),
    ...
    ('p', 'Several allowed, separated by ;. From DevTools > Application > Cookies. '
          'Sent verbatim on every request; the GUI shows it for confirmation first.'),
    ('h', 'How a hit is decided'),
    ('p', 'Pick a Detection method. "Fail keyword" is the most reliable: type the '
          'word on a FAILED login (e.g. Invalid); any response WITHOUT that word is '
          'a success. "Success keyword" is the opposite. The status options match '
          'on HTTP status (302, 401). Leave it on Auto to let the tool scan and '
          'baseline by itself.'),
    ('h', 'Politeness'),
    ...
    ('p', '5. Cookie = PHPSESSID=your-live-value if the login needs one.'),
...
class App(tk.Tk):
    ...
    def _build_header(self):
        ...
        self._w_title = tk.Label(left, text='🔓 Auth-Hunter', bg=p['BG'], fg=p['ACCENT'],
                                 font=('Segoe UI Semibold', 18))
        self._w_title.pack(anchor='w', padx=16, pady=(12, 0))
        self._w_sub = tk.Label(left, text='Exam-grade login brute forcer  -  graphical front-end',
    ...
    def _build_form(self, form):
        ...
        self._e(lf, 1, 'Login URL', self.v_url, 'e.g. https://site/login.php',
                tip='The form action, the page the login POSTs to. Read it from '
                    '<form action="..."> in the page HTML.')
        ...
        self._e(lf, 0, 'Single user (-l)', self.v_user, 'e.g. admin',
                tip='One username to try against every password. A VALUE you try, '
                    'not the field name. For a list use User list (-L).')
        ...
        self._e(lf, 2, 'Single password (-p)', self.v_pass, 'spray one password',
                tip='One password tried for every user. For a wordlist use Password '
                    'list (-P).')
        ...
        self._e(lf, 5, 'User field name', self.v_userparam, 'default: username',
                tip='The NAME of the username input in the form HTML, e.g. '
                    '<input name="username">. Leave blank unless it differs (e.g. email).')
        self.v_passparam = tk.StringVar()
        self._e(lf, 6, 'Pass field name', self.v_passparam, 'default: password',
                tip='The NAME of the password input in the form HTML, e.g. '
                    '<input name="password">. Leave blank unless it differs (e.g. pwd).')
        ...
        self._e(lf, 1, 'Value', self.v_detectval, 'keyword or status code for the method',
        ...
        self.t_extra = self._text(lf, 3, 'Extra params', 3, 'one per line, "key=value"')
        self.v_threads = tk.StringVar(value='8')
        self._e(lf, 4, 'Threads (-t)', self.v_threads, 'default 8, hard cap 20',
        ...
        self._e(lf, 5, 'Delay (-d)', self.v_delay, 'seconds between requests per thread',
        ...
        self._e(lf, 6, 'Timeout', self.v_timeout, 'per request, seconds',
        ...
        self._c(lf, 7, 'Skip TLS certificate verification (--insecure)', self.v_insecure,
    ...
    def _example_request(self):
        if self.v_target.get() == 'request':
            ...
            shown = content.replace('§USER§', 'admin').replace('§PASS§', 'Password1')
            shown = shown.replace('FUZZUSER', 'admin').replace('FUZZPASS', 'Password1')
            return ('# Your request file is sent as-is, with the markers replaced by\n'
                    '# each credential pair. One request per attempt. Sample shown: admin / Password1\n'
                    '# ----------------------------------------------------------------\n\n%s' % shown)

        url = self.v_url.get().strip()
        if not url:
            return 'Enter the Login URL first.'
        method = self.v_method.get()
        user = self.v_user.get().strip() or 'admin'
        pwd = self.v_pass.get().strip() or 'Password1'
        uf = self.v_userparam.get().strip() or 'username'
        pf = self.v_passparam.get().strip() or 'password'
        ...
        pairs = list(self._extra_params()) + [(uf, user), (pf, pwd)]
        encoded = urllib.parse.urlencode(pairs)
        if method == 'GET':
            sep = '&' if query else '?'
            full = path + (('?' + query) if query else '') + sep + encoded
            body = ''
        else:
            full = path + (('?' + query) if query else '')
            body = encoded

        headers = [('Host', host), ('User-Agent', 'Mozilla/5.0 (Auth-Hunter/1.0)'),
        ...
        out = ['# Example of ONE attempt the tool sends (sample creds: %s / %s).' % (user, pwd),
               '# The real run sends one like this per credential pair.',
               '# ----------------------------------------------------------------', '']
    ...
    def on_help(self):
        ...
        win.transient(self)
        tk.Label(win, text='Auth-Hunter  ·  Field Guide', bg=p['BG'], fg=p['ACCENT'],
